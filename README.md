# 📚 Python & Data Science Assignments Directory

## 📋 Overview Table

| File Name | Topic / Focus | Questions / Tasks | Core Libraries & Concepts |
| :--- | :--- | :--- | :--- |
| [**`Assignment_1.ipynb`**](./Assignment_1.ipynb) | Python Basics & String Indexing | 10 Slicing Tasks | Variables, Data Types (`int`, `float`, `str`), String Indexing & Slicing |
| [**`Assignment_2.ipynb`**](./Assignment_2.ipynb) | Multi-Dimensional Data Structures | 50+ Record Retrieval Tasks | 3D Nested Lists, Banking Records Navigation, Data Extraction |
| [**`Assignment_3.ipynb`**](./Assignment_3.ipynb) | Python Built-in Functions | 50 Practice Exercises | `abs()`, `round()`, `pow()`, `min()`, `max()`, `zip()`, `enumerate()`, `isinstance()` |
| [**`Assignment_4.ipynb`**](./Assignment_4.ipynb) | Dynamic Arguments (`*args` & `**kwargs`) | 20 Coding Fill-in-the-Blanks | `*args`, `**kwargs`, Argument Unpacking, Dynamic Function Signatures |
| [**`Assignment_6.ipynb`**](./Assignment_6.ipynb) | Functional Programming Mastery | 100 Practice Questions | `lambda`, `map()`, `filter()`, `reduce()`, Functional Pipelines |
| [**`Assignment_7.ipynb`**](./Assignment_7.ipynb) | Masterclass on Functions & Scope | 100 Structured Assignments | Function Definitions, Parameters, Closures, LEGB Scope (`global`/`nonlocal`) |
| [**`Assignment_8.ipynb`**](./Assignment_8.ipynb) | NumPy Retail Analytics Case Study | 80+ Analytical Questions | NumPy Arrays, Matrix Aggregations, Boolean Masking, Business Analytics |
| [**`python-control-flow-100.ipynb`**](./python-control-flow-100.ipynb) | Python Control Flow (Edukron) | 100 Examples + 100 Assignments | `if/elif/else`, `for` loops, `while` loops, Nested Logic, Capstone Validation |